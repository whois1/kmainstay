...
func TestBootstrap_CreatesInitialHumanOrganisationAndGeneralExactlyOnce(t *testing.T) {
	...
	if err := db.QueryRow(`SELECT u.kind,u.email,u.name,c.name,m.role,m.name_normalized FROM users u JOIN organisation_memberships m ON m.user_id=u.id JOIN conversations c ON c.organisation_id=m.organisation_id WHERE u.id=?`, got.UserID).Scan(&kind, &email, &name, &conversation, &role, &normalisedName); err != nil {
		t.Fatal(err)
	}
	if kind != "human" || email != "michael@example.com" || name != "Michaél" || conversation != "general" || role != "admin" || normalisedName != "michaél" {
		t.Fatalf("got %q %q %q %q %q %q", kind, email, name, conversation, role, normalisedName)
	}
